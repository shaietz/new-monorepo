...
function makeNameValidator(root: string) {
    return (input: string): true | string => {
        ...
        for (const dir of WORKSPACE_DIRS) {
            if (existsSync(path.join(root, dir, name)))
                return `${dir}/${name} already exists`;
        }
...
function makePortValidator(root: string) {
    return (input: string): true | string => {
        ...
        if (port < 1024 || port > LAST_PORT)
            return `The port must be between 1024 and ${LAST_PORT}`;

        const owner = scanPorts(root).get(port);
        return owner === undefined
            ? true
            : `Port ${port} is already taken by apps/${owner}`;
    };
}
 ...
 * complete.
 */
const installAction: PlopTypes.CustomActionFunction = (
    _answers,
    _config,
    plop,
) => {
    ...
    if (result.status !== 0) {
        throw new Error(
            `npm install exited with ${String(result.status)} — run it by hand to see why`,
        );
    }
...
export default function generator(plop: PlopTypes.NodePlopAPI): void {
    ...
    plop.setGenerator('fastify-service', {
        description:
            'A Fastify 5 service in apps/, built on @repo/fastify-base',
        prompts: [
            {
                type: 'input',
                name: 'name',
    ...
    plop.setGenerator('package', {
        description:
            'A shared package in packages/, published internally as @repo/<name>',
        prompts: [
            {
                type: 'input',
                name: 'name',
                message:
                    'Package name (kebab-case; becomes packages/<name>, imported as @repo/<name>)',
                validate: validateName,
            },
            {
                type: 'list',
